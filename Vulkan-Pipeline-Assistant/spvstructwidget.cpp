...
SpvStructWidget::SpvStructWidget(SpvStructType* type, QWidget* parent)
    ...
    for (int i = 0; i < m_typeWidgets.size(); ++i) {
        if (m_type->members[i]->Type() == SpvTypeName::VECTOR) {
            m_typeWidgets[i] = new SpvVectorWidget((SpvVectorType*)m_type->members[i], parent);
        }
        else if (m_type->members[i]->Type() == SpvTypeName::MATRIX) {
            m_typeWidgets[i] = new SpvMatrixWidget((SpvMatrixType*)m_type->members[i], parent);
        }
        else if (m_type->members[i]->Type() == SpvTypeName::STRUCT) {
            m_typeWidgets[i] = new SpvStructWidget((SpvStructType*)m_type->members[i], parent);
        }
        else if (m_type->members[i]->Type() == SpvTypeName::ARRAY) {
            m_typeWidgets[i] = new SpvArrayWidget((SpvArrayType*)m_type->members[i], parent);
        }
        else {
...
void SpvStructWidget::Fill(unsigned char* data) {
    size_t offset = 0;
    for (int i = 0; i < m_typeWidgets.size(); ++i) {
        offset = m_type->memberOffsets[i];
